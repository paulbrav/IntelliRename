    ...
    try:
        with open(pdf_path, "rb") as f:
            ...
            if len(pdf.pages) > 0:
                ...
                title_candidates = [line for line in candidate_lines if 3 < len(line) < 100]
        ...
        with open(pdf_path, "rb") as f:
            ...
            if len(pdf.pages) > 0:
                ...
                for i in range(1, min(5, len(lines))):
                    ...
                    if re.search(r'University|Institute|College|Department', line, re.IGNORECASE):
...
def extract_year_from_content(pdf_path: str) -> Optional[str]:
    ...
    try:
        with open(pdf_path, "rb") as f:
            ...
            copyright_match = re.search(r'copyright\s+©?\s*(\d{4})', all_text, re.IGNORECASE)
            if copyright_match:
                return copyright_match.group(1)
            
            # Look for year with publication context
            pub_year_match = re.search(r'published\s+in\s+(\d{4})', all_text, re.IGNORECASE)
            if pub_year_match:
                return pub_year_match.group(1)
            
            # Try to find conference/journal with year
            conf_match = re.search(r'proceedings\s+of.*?(\d{4})', all_text, re.IGNORECASE)
            if conf_match:
            ...
            if year_matches:
                ...
                valid_years = [int(y) for y in year_matches if 1900 <= int(y) <= current_year]
                if valid_years:
...
def extract_advanced_metadata(pdf_path: str) -> Dict[str, str]:
    ...
    """
    from pdf_renamer.main import UNKNOWN_AUTHOR, UNKNOWN_TITLE, UNKNOWN_YEAR
