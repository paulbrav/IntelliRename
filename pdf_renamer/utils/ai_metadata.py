...
try:
    from pdf_renamer.utils.cache import get_cache_key, get_from_cache, save_to_cache
...
env_locations = [
    ...
    os.path.join(os.getcwd(), 'pdf_renamer', '.env'),  # pdf_renamer subdirectory
...
if PERPLEXITY_API_KEY:
    ...
    if not PERPLEXITY_API_KEY.startswith('pplx-'):
        log.warning("Perplexity API key appears to be in an invalid format. Keys should start with 'pplx-'")
        log.info("Please check your API key at https://www.perplexity.ai/settings/api")
    else:
        log.info(f"Perplexity API key found. First 5 chars: {PERPLEXITY_API_KEY[:5]}...")
else:
...
def evaluate_metadata_quality(metadata: Dict[str, str], unknown_markers: Dict[str, str]) -> Tuple[float, Dict[str, float]]:
    """
...
def query_perplexity(metadata: Dict[str, str], filename: str) -> Dict[str, str]:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            log.info(f"Querying Perplexity API for metadata enhancement (attempt {attempt+1})")
            ...
            if response.status_code == 401:
                ...
                log.error("401 Unauthorized: Invalid or expired API key, or insufficient credits")
                log.error("Please check your Perplexity API key and account status at https://www.perplexity.ai/settings/api")
    ...
    if not response_data:
        log.warning("Failed to get response from Perplexity API after multiple attempts")
        return metadata
...
def clean_garbled_metadata(metadata: Dict[str, str]) -> Dict[str, str]:
    ...
    if result.get("title"):
        ...
        result["title"] = re.sub(r'[\[\(](?:PDF|EPUB|ebook|Ebook)[\]\)]', '', result["title"])
...
def enhance_metadata(
    ...
    confidence_score, field_scores = evaluate_metadata_quality(metadata, unknown_markers)
    ...
    log.info(f"Metadata quality score {confidence_score:.2f} below threshold {confidence_threshold}. Using AI enhancement.")
